...
### Load Data ###
raw.dat.full<-rbind(
      #read.xlsx("./Output/Modeling/Week_3_year_2023_Data.xlsx"),
...
cor(mod.dat$home_win, mod.dat$win_pct_diff) #0.61
...
cor(mod.dat$home_win, mod.dat$SRS_diff)#0.63
...
cor(mod.dat$home_win, mod.dat$sp_plus_diff)#0.58
...
cor(mod.dat$home_win, mod.dat$postgame_wp_diff)#0.45 (Better but not great)
...
# Create Power Rank Difference
mod.dat$Pwr_rk_diff<-mod.dat$Pwr_Rk_home-mod.dat$Pwr_Rk_away
cor(mod.dat$home_win, mod.dat$Pwr_rk_diff)#0.46
...
# Drop variables not useful after examination
mod.dat<-mod.dat%>%
...
confusionMatrix(as.factor(mod1.test.dat$predictions),as.factor(mod1.test.dat$home_win)) #87.4% Accuraccy
prop.table(table(Prediction = mod1.test.dat$predictions, Actual = mod1.test.dat$home_win))%>%round(2)
#Inaccuracies: 7% false positive, 6% false negative

summary(fit.1)

##### Trying other models - Commenting out as not as good #####
#
